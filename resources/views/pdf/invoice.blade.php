...
<div class="container">
    ...
    <table>
        ...
        @foreach($order->items as $item)
            <tr>
                <td>{{ $item->burger ? $item->burger->name : 'Burger indisponible' }}</td>
                <td>{{ $item->quantity }}</td>
    ...
    <div class="totals">
        <div><strong>Total HT:</strong> {{ number_format($order->total_amount / 1.2, 2) }} CFA</div>
        <div><strong>TVA (20%):</strong> {{ number_format($order->total_amount - ($order->total_amount / 1.2), 2) }} CFA</div>
        <div class="total">TOTAL TTC: {{ number_format($order->total_amount, 2) }} CFA</div>
    </div>
